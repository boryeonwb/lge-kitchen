    ...
    <>
      ...
      <div className="overflow-hidden rounded-[12px] bg-paper shadow-soft">
        <DataTable
          ...
          expanded={open}
          onToggle={toggleRow}
          renderDetail={(r) => <MonthDetail r={r as CarryRow} />}
        />
      </div>
    </>
  )
...
/** 펼친 행의 월별 소진 — 표를 두 열로 접은 대신 여기서 달별로 보여준다 */
function MonthDetail({ r }: { r: CarryRow }) {
  const months = Object.keys(r.monKrw || {}).sort((a, b) => +a - +b)
  if (!months.length)
    return <div className="px-6 py-4 text-[12px] text-fog">이 라인은 아직 소진 기록이 없습니다.</div>
  return (
    <div className="px-6 py-4">
      <div className="mb-2 text-[12px] text-fog">
        {r.sol} · {r.ctry} · {r.phase || "(구분없음)"} · {r.med} — 월별 소진
        {r.sheetRatio && (r.sheetRatio < 0.75 || r.sheetRatio > 1.35) ? (
          <span className="tint tint-warn ml-2 text-[11.5px]">
            ⚠ 시트가 인보이스의 {Math.round(r.sheetRatio * 100)}% — 시트로 메운 달을 확인하세요
          </span>
        ) : null}
      </div>
      <div className="flex flex-wrap gap-2">
        {months.map((m) => (
          <div key={m} className="min-w-[132px] rounded-[12px] bg-cream px-3.5 py-2.5">
            <div className="text-[11px] text-fog">{m}월</div>
            <div className="mt-0.5 text-[13px] font-semibold tabular-nums">
              {f0(r.monKrw[m])}
              <span className="ml-1 text-[11px] font-normal text-fog">원</span>
            </div>
            <div className="text-[11.5px] tabular-nums text-fog">
              {r.monNat[m] !== undefined ? `${f2(r.monNat[m])} ${r.setCur}` : ""}
            </div>
            <div className="mt-1">
              <SrcValue src={r.monSrc[m]} mon={m} text={r.monSrc[m] || "?"} />
            </div>
          </div>
        ))}
      </div>
