...
REM   1) 정산 백엔드(:8010)  2) 이 앱의 정적 서버(:3100, /api 는 백엔드로 프록시)
REM 8000 이 아니라 8010 을 쓴다 — 내부 정산 대시보드가 8000 을 쓰고 있어서,
REM 둘 다 켜면 나중에 뜬 쪽이 포트를 못 잡고 조용히 죽는다(화면은 껍데기만 뜬다).
...
echo [1/2] 정산 백엔드 :8010
start "LGE 정산 백엔드" cmd /k "cd /d %BACKEND% && set LGE_WORK=%LGE_WORK%&& set LGE_STATE=%LGE_STATE%&& set LGE_PROJECT=%LGE_PROJECT%&& python -m uvicorn main:app --host 0.0.0.0 --port 8010"

echo      기동 대기...
timeout /t 6 /nobreak >nul

echo [2/2] 대시보드 :3100
start "LGE 광고주 대시보드" cmd /k "cd /d %~dp0 && python serve_local.py 3100 http://127.0.0.1:8010"
